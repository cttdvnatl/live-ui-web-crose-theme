const vietHongDocuments = [
    {   
        fileName: 'School Year Registration Form (NEW student)',
        downloadFile: 'https://hvmatl.net/gallery/kgd/viethong/Viet-Hong-School-2021-2022-NEW-Student-Registration-Form.pdf',
        description: 'Đơn Ghi Danh Niên Học 2021-2022',
    },
    {   
        fileName: 'Parent Handbook (Digital Learning)',
        downloadFile: 'https://hvmatl.net/gallery/kgd/viethong/Viet-Hong-School-2020-2021-Parents-Handbook-bilingual.pdf',
        description: 'Sổ Tay Phụ Huynh 2020-2021',
    },
    {   
        fileName: 'Policies for Parents and Students (Digital Learning)',
        downloadFile: 'https://hvmatl.net/gallery/kgd/viethong/Viet-Hong-School-2020-2021-Digital-Learning-Policies-Parents-Students.pdf',
        description: 'Nội Quy cho Phụ Huynh và Học Sinh',
    },
    {   
        fileName: 'Typing in Vietnamese Using the Telex Keyboard Tutorial',
        downloadFile: 'https://hvmatl.net/gallery/kgd/viethong/Viet-Hong-School-2020-2021-Typing-Vietnamese-Using-Telex-bilingual.pdf',
        description: 'Đánh Chữ Tiếng Việt Bằng Bảng Telex',
    },
    {   
        fileName: 'School Year Calendar',
        downloadFile: 'https://hvmatl.net/gallery/kgd/viethong/Viet-Hong-School-2020-2021-School-Year-Calendar.pdf',
        description: 'Lịch Trình Niên Học 2020-2021',
    },
    {   
        fileName: 'Student Absence Form',
        downloadFile: 'https://hvmatl.net/gallery/kgd/viethong/Viet-Hong-School-2020-2021-Absence-Form.pdf',
        description: 'Giấy Xin Phép Vắng Mặt',
    },
    {   
        fileName: 'Permission to Contact Youth',
        downloadFile: 'https://docs.google.com/forms/d/e/1FAIpQLScNXxHFs6fdvLQWp2tJ45VJP37MOWO8Sy2czea6F1-7TtZEFg/viewform',
        description: 'Giấy Phép Xin Liên Lạc Với Trẻ Vị Thành Niên',
    },
]

export default vietHongDocuments;