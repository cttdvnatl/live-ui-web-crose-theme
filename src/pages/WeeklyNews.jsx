...
const WeeklyNews = (props) => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("token") && !props.data.length) {
      ...
      (async () =>
        await props.getWeeklyNews(from, to, sessionStorage.getItem("token")))();
    }
  }, [props]);
              ...
              <div className="row">
                <div className="col-12">
                  <div className="section-heading">
                    <h2>
                      <b>Thông tin mục vụ tháng {month}</b>
                    </h2>
                  </div>
                </div>
              </div>
              <div className="row about-content justify-content-center">
                {props.data.map((weeklyNews, idx) =>
                  new Date(weeklyNews.date).getMonth() + 1 === month ? (
                    <div
                    ...
                    >
                      <div className="about-us-content mb-100">
                        <a
                          href="/"
                          onClick={(e) =>
                            toggleModal(e, weeklyNews.title, weeklyNews.src)
                          }
                        >
                          <img src={weeklyNews.image} alt="" />
                        </a>
...
const mapDispatchToProps = (dispatch) => ({
  getWeeklyNews: (fromDate, toDate, token = null) =>
    getWeeklyNews(dispatch, fromDate, toDate, token),
  toggleModal: (content) =>
    dispatch({ type: actionType.TOGGLE_WEEKLY_NEWS_POPUP, content: content }),
});
